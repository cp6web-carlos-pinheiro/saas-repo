...
<div class="w-full p-5 md:p-8">
    <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            <x-ui.button :href="route('inventory.plants.index')" variant="secondary" class="rounded-full">{{ __('ui.back') }}</x-ui.button>
            <x-ui.button :href="route('inventory.plants.edit', $plant)" variant="primary" class="rounded-full">{{ __('plant.edit') }}</x-ui.button>
            <form method="POST" action="{{ route('inventory.plants.destroy', $plant) }}" data-admin-delete-confirm data-admin-name="{{ $plant->name }}" data-confirm-title="{{ __('plant.confirm_delete_title') }}" data-confirm-text="{{ __('plant.confirm_delete_text') }}" data-confirm-confirm="{{ __('plant.confirm_delete_confirm') }}" data-confirm-cancel="{{ __('plant.confirm_delete_cancel') }}">
                @csrf
                @method('DELETE')
                <x-ui.button type="submit" variant="danger" class="rounded-full">{{ __('plant.remove') }}</x-ui.button>
            </form>
        </div>
    </div>
