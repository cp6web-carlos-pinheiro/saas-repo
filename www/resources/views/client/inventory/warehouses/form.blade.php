...
<div class="w-full p-5 md:p-8">
    <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <x-ui.button :href="$editing ? route('inventory.warehouses.show', $warehouse) : route('inventory.warehouses.index')" variant="material-back" class="rounded-full">{{ __('ui.back') }}</x-ui.button>
    </div>
    ...
    <x-ui.panel class="mt-6 border-[#dadce0] shadow-none" padding="p-6 md:p-8">
        <form method="POST" action="{{ $editing ? route('inventory.warehouses.update', $warehouse) : route('inventory.warehouses.store') }}" class="space-y-5">
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <label class="block text-sm font-medium">
                    {{ __('warehouse.code') }}
                    <x-ui.input name="code" :value="old('code', $warehouse?->code)" class="mt-2" required />
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <label class="block text-sm font-medium">
                    ...
                    @error('plant_id')<span class="mt-1 block text-sm text-red-700">{{ $message }}</span>@enderror
                </label>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
                <x-ui.button :href="$editing ? route('inventory.warehouses.show', $warehouse) : route('inventory.warehouses.index')" variant="material-back" class="rounded-full" :full="true">{{ __('ui.back') }}</x-ui.button>
                <x-ui.button type="submit" variant="brand-primary" class="rounded-full" :full="true">{{ $editing ? __('warehouse.save') : __('warehouse.create') }}</x-ui.button>
            </div>
        </form>
    </x-ui.panel>
</div>
