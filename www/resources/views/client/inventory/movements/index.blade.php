            ...
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    @forelse ($movements as $movement)
                        <tr class="border-b border-[#f1f3f4]"><td class="px-3 py-4">{{ $movement->movement_at?->format('d/m/Y H:i') }}</td><td class="px-3 py-4">{{ $movementTypes[$movement->movement_type] ?? $movement->movement_type }}</td><td class="px-3 py-4">{{ $movement->warehouse?->code }}</td><td class="px-3 py-4">{{ $movement->product?->sku }} — {{ $movement->product?->description }}</td><td class="px-3 py-4 text-right">{{ number_format($movement->quantity, 3, ',', '.') }} {{ $movement->product?->unit?->code }}</td><td class="px-3 py-4 text-[#5f6368]">{{ $movement->reference_type ? $movement->reference_type.' #'.$movement->reference_id : '—' }}</td><td class="px-3 py-4 text-[#5f6368]">{{ $movement->notes ?: '—' }}</td></tr>
                    @empty
                        <tr><td colspan="7" class="px-3 py-10 text-center text-[#5f6368]">{{ __('inventory_web.empty_movements') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
