...
<div class="w-full p-5 md:p-8"><div class="flex items-center justify-between gap-4"><h1 class="font-display text-3xl font-bold">{{ __('inventory_web.new_movement') }}</h1><x-ui.button :href="route('inventory.movements.index')" variant="secondary" class="rounded-full">{{ __('inventory_web.back') }}</x-ui.button></div><x-ui.panel class="mt-6 max-w-4xl border-[var(--ui-border)] shadow-none" padding="p-5 md:p-6"><form method="POST" action="{{ route('inventory.movements.store') }}" class="grid gap-5 md:grid-cols-2">@csrf
