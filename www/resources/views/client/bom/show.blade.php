...
<div class="w-full p-5 md:p-8">
    <x-ui.page-heading title="{{ $bom->product?->sku ?? __('bom.title') }}" subtitle="{{ $bom->product?->description ?? '—' }}">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <x-ui.confirm-button :action="route('bom.material-lists.destroy', $bom)" method="DELETE" class="rounded-full" :label="__('bom.remove')" :confirm-title="__('bom.confirm_delete_title')" :confirm-text="__('bom.confirm_delete_text')" :confirm-label="__('bom.confirm_delete_confirm')" :cancel-label="__('bom.confirm_delete_cancel')" />
        </div>
        </x-slot:actions>
    </x-ui.page-heading>
