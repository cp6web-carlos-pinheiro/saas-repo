...
<div class="w-full p-5 md:p-8">
    ...
    <x-ui.panel class="mt-6 border-[#dadce0] shadow-none" padding="p-5 md:p-6">
        <form class="flex gap-3" method="GET">
            ...
            <input id="supplier-search" name="search" value="{{ $search }}" class="min-w-0 flex-1 rounded-xl border border-[#dadce0] px-4 py-3" placeholder="{{ __('supplier.search') }}">
            <x-ui.button type="submit" variant="surface-muted" class="rounded-xl">{{ __('supplier.filter') }}</x-ui.button>
        </form>

        @php($sortUrl = fn ($column) => route('purchasing.suppliers.index', ['search' => $search, 'sort' => $column, 'direction' => $sort === $column && $direction === 'asc' ? 'desc' : 'asc']))
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-[#dadce0] text-left text-[#5f6368]">
                        ...
                        <th class="px-3 py-3"><a href="{{ $sortUrl('code') }}">{{ __('supplier.code') }} ↕</a></th>
                ...
                <tbody>
                    @forelse ($suppliers as $supplier)
                        ...
                        >
                            ...
                            <td class="px-3 py-4 font-semibold">{{ $supplier->code }}</td>
