...
<div class="w-full p-5 md:p-8">
    ...
    <x-ui.panel class="mt-6 border-[var(--ui-border)] shadow-none" padding="p-6 md:p-8">
        <form method="POST" action="{{ $editing ? route('customers.update', $customer) : route('customers.store') }}" class="space-y-5">
            ...
            <label class="block text-sm font-medium">
                {{ __('customer.email') }}
                <x-ui.input name="email" type="email" :value="old('email', $customer?->email)" @class(['mt-2', 'border-[var(--ui-danger)]' => $errors->has('email'), 'border-[var(--ui-border)]' => ! $errors->has('email')]) />
                @error('email')<span class="mt-1 block text-sm text-[var(--ui-danger)]">{{ $message }}</span>@enderror
            </label>

            <label class="block text-sm font-medium">
                {{ __('customer.phone') }}
                <x-ui.input name="phone" :value="old('phone', $customer?->phone)" @class(['mt-2', 'border-[var(--ui-danger)]' => $errors->has('phone'), 'border-[var(--ui-border)]' => ! $errors->has('phone')]) />
