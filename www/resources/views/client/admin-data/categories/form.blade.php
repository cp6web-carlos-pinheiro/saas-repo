...
@section('client-page-title', $editing ? __('admin_data_categories.edit') : __('admin_data_categories.create'))

@section('client-content')
<div class="w-full p-5 md:p-8">
    <div class="flex flex-wrap items-end justify-between gap-4">
        <h1 class="font-display text-3xl font-bold">{{ __('admin_data_categories.title') }}</h1>
        <x-ui.button :href="$editing ? route('admin-data.categories.show', $category) : route('admin-data.categories.index')" variant="material-back" class="rounded-full">{{ __('ui.back') }}</x-ui.button>
    </div>
    ...
    <x-ui.panel class="mt-6 border-[#dadce0] shadow-none" padding="p-6 md:p-8">
        <form method="POST" action="{{ $editing ? route('admin-data.categories.update', $category) : route('admin-data.categories.store') }}" class="space-y-5">
            @csrf
            @if ($editing)
                @method('PUT')
            @endif
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
                <x-ui.button :href="$editing ? route('admin-data.categories.show', $category) : route('admin-data.categories.index')" variant="material-back" class="rounded-full" :full="true">{{ __('ui.back') }}</x-ui.button>
                <x-ui.button type="submit" variant="brand-primary" class="rounded-full" :full="true">{{ $editing ? __('admin_data_categories.save') : __('admin_data_categories.create') }}</x-ui.button>
            </div>
        </form>
    </x-ui.panel>
</div>
