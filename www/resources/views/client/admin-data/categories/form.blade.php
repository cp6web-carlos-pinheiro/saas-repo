    ...
    <x-ui.panel class="mt-6 border-[var(--ui-border)] shadow-none" padding="p-6 md:p-8">
        <form method="POST" action="{{ $editing ? route('admin-data.categories.update', $category) : route('admin-data.categories.store') }}" class="space-y-5">
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
                ...
                <x-ui.button type="submit" variant="primary" class="rounded-full" :full="true">{{ $editing ? __('admin_data_categories.save') : __('admin_data_categories.create') }}</x-ui.button>
            </div>
        </form>
    </x-ui.panel>
</div>
