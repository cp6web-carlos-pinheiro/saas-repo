...
<div class="w-full p-5 md:p-8">
    <x-ui.page-heading title="{{ __('admin_data_brands.title') }}">
        <x-slot:actions>
        <x-ui.button :href="$editing ? route('admin-data.brands.show', $brand) : route('admin-data.brands.index')" variant="secondary" class="rounded-full">{{ __('ui.back') }}</x-ui.button>
        </x-slot:actions>
    </x-ui.page-heading>
    ...
    <x-ui.panel class="mt-6 border-[var(--ui-border)] shadow-none" padding="p-6 md:p-8">
        <form method="POST" action="{{ $editing ? route('admin-data.brands.update', $brand) : route('admin-data.brands.store') }}" class="space-y-5">
            ...
            <x-ui.field label="{{ __('admin_data.name') }}" for="name" :required="true" :error="$errors->first('name')">
                <x-ui.input name="name" :value="old('name', $brand?->name)" class="mt-2" required  id="name" :aria-describedby="$errors->has('name') ? 'name-error' : null"/>
            </x-ui.field>

            <x-ui.field label="{{ __('admin_data.description') }}" for="description" :error="$errors->first('description')">
                <x-ui.textarea name="description" class="mt-2" rows="5" id="description" :aria-describedby="$errors->has('description') ? 'description-error' : null">{{ old('description', $brand?->description) }}</x-ui.textarea>
            </x-ui.field>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
                <x-ui.button :href="$editing ? route('admin-data.brands.show', $brand) : route('admin-data.brands.index')" variant="secondary" class="rounded-full" :full="true">{{ __('ui.back') }}</x-ui.button>
                <x-ui.button type="submit" variant="primary" class="rounded-full" :full="true">{{ $editing ? __('admin_data_brands.save') : __('admin_data_brands.create') }}</x-ui.button>
            </div>
        </form>
    </x-ui.panel>
</div>
