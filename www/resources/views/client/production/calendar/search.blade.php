...
<div class="w-full p-5 md:p-8">
    <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <x-ui.button :href="route('production.calendar.create')" variant="brand-primary" class="rounded-full">{{ __('production.calendar.new_short') }}</x-ui.button>
    </div>
    ...
    <x-ui.panel class="mt-6 border-[#dadce0] shadow-none" padding="p-5 md:p-6">
        <form class="grid gap-4 sm:grid-cols-3 lg:grid-cols-5" method="GET">
            ...
            <div class="flex items-end">
                <x-ui.button type="submit" variant="surface-muted" class="rounded-full" :full="true">{{ __('production.filter') }}</x-ui.button>
            </div>
        </form>
        ...
        <form method="POST" action="{{ route('production.calendar.generate') }}" class="mt-4">
            ...
            <x-ui.button type="submit" variant="brand-primary" class="rounded-full">{{ __('production.calendar.generate') }}</x-ui.button>
        </form>
        ...
        <div class="mt-6 overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="5" class="px-3 py-6 text-center text-[#5f6368]">{{ __('production.calendar.empty') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
