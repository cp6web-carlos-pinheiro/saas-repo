...
<div class="w-full p-5 md:p-8">
    <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <x-ui.button :href="route('production.work-centers.edit', $workCenter)" variant="material-edit" class="rounded-full">{{ __('production.edit') }}</x-ui.button>
        </div>
    </div>
        ...
        <x-ui.panel class="border-[#dadce0] shadow-none" padding="p-6">
            ...
            <form method="POST" action="{{ route('production.work-centers.shifts.store', $workCenter) }}" class="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <label class="inline-flex items-center gap-2 text-sm sm:col-span-2">
                    <input type="checkbox" name="is_active" value="1" checked /> {{ __('production.active') }}
                </label>
                <x-ui.button type="submit" variant="brand-primary" class="rounded-full sm:col-span-2">{{ __('production.work_centers.save_shift') }}</x-ui.button>
            </form>
        </x-ui.panel>
            ...
            <div class="mt-4 space-y-2 text-sm">
                @forelse ($workCenter->shifts as $shift)
                    <div class="rounded-xl border border-[#dadce0] p-3">
                        ...
                        <div class="text-[#5f6368]">{{ __('production.work_centers.shift_summary', ['capacity' => number_format((float) $shift->capacity_hours, 2, ',', '.'), 'status' => $shift->is_active ? __('production.active') : __('production.inactive')]) }}</div>
                    </div>
                @empty
                    <div class="text-[#5f6368]">{{ __('production.work_centers.no_shifts') }}</div>
                @endforelse
            </div>
        </x-ui.panel>
    </div>
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="4" class="px-3 py-6 text-center text-[#5f6368]">{{ __('production.work_centers.no_days') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
