    ...
    <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            @if ($version->status === 'DRAFT')
                <x-ui.button :href="route('production.routing.edit', $version)" variant="material-edit" class="rounded-full">{{ __('production.edit') }}</x-ui.button>
            @endif
        </div>
    </div>
        ...
        @if ($version->status === 'DRAFT')
            <form method="POST" action="{{ route('production.routing.approve', $version) }}" class="mt-6 grid gap-3 sm:grid-cols-3">
                ...
                <div class="flex items-end">
                    <x-ui.button type="submit" variant="brand-primary" class="rounded-full" :full="true">{{ __('production.routing.approve') }}</x-ui.button>
                </div>
            </form>
        @endif
    </x-ui.panel>
        ...
        <x-ui.panel class="mt-6 border-[#dadce0] shadow-none" padding="p-6">
            ...
            <form method="POST" action="{{ route('production.routing.operations.store', $version) }}" class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <label class="inline-flex items-center gap-2 self-end pb-2 text-sm">
                    <input type="checkbox" name="is_outsourced" value="1" /> {{ __('production.routing.outsourced') }}
                </label>
                <div class="lg:col-span-2 flex items-end">
                    <x-ui.button type="submit" variant="brand-primary" class="rounded-full" :full="true">{{ __('production.routing.add_operation') }}</x-ui.button>
                </div>
            </form>
        </x-ui.panel>
    @endif
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="6" class="px-3 py-6 text-center text-[#5f6368]">{{ __('production.routing.empty_operations') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
