...
@section('content')
  <div class="min-h-screen md:grid md:grid-cols-[280px_1fr] md:transition-[grid-template-columns] md:duration-300" data-admin-sidebar-shell>
    <aside class="hidden border-r border-[#dadce0] bg-white p-5 md:flex md:flex-col md:transition-all md:duration-300" data-admin-sidebar>
      ...
      <nav class="mt-8 space-y-1" aria-label="{{ __('global_admin.title') }}" data-admin-sidebar-content>
        ...
        @php($links = [['home', route('global-admin.home'), '⌂'], ['modules.customers', route('global-admin.customers.index'), '♙'], ['modules.companies', route('global-admin.companies.index'), '▣'], ['modules.plans', route('global-admin.modules.show', 'plans'), '◇'], ['modules.administrators', route('global-admin.administrators.index'), '⚙']])
      ...
      <div class="mt-auto border-t border-[#dadce0] pt-5" data-admin-sidebar-content>
