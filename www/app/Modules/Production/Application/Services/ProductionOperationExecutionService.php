...
final class ProductionOperationExecutionService extends BaseService
{
    ...
    public function command(int $operationId, string $eventType, array $payload, ?int $userId = null): array
    {
        ...
        if ($key === '') {
            throw new DomainException('idempotency_key is required for MES commands', 422);
        }
        ...
        if (in_array($operation->status, ['COMPLETED', 'CANCELLED'], true)) {
            throw new DomainException('Operation is already closed', 422);
        }
        if (in_array($eventType, ['PAUSE', 'STOP', 'CANCEL'], true) && empty($payload['reason_code'])) {
            throw new DomainException('reason_code is required for this command', 422);
        }
        ...
        if ($resourceId) {
            ...
            if ((int) $resource->work_center_id !== (int) $operation->work_center_id || $resource->status !== 'ACTIVE') {
                throw new DomainException('Selected resource is not active or does not belong to the operation work center', 422);
            }
            if ($eventType === 'START') {
                ...
                if ($busy) {
                    throw new DomainException('Selected resource already has an active operation', 409);
                }
            }
        }
        ...
        $this->inTransaction(function () use ($operation, $eventType, $payload, $userId, $key, $occurredAt, $resourceId): void {
            ...
            $status = match ($eventType) {
                'START','RESUME' => 'IN_PROGRESS', 'PAUSE' => 'PAUSED', 'STOP' => 'STOPPED', 'COMPLETE' => 'COMPLETED', 'CANCEL' => 'CANCELLED', default => $operation->status
            };
            $operation->status = $status;
            $operation->actual_production_resource_id = $resourceId;
            $operation->operator_id = $payload['operator_id'] ?? $userId;
            if ($eventType === 'START') {
                $operation->actual_started_at ??= $occurredAt;
            }
            if (in_array($eventType, ['COMPLETE', 'CANCEL'], true)) {
                $operation->actual_completed_at = $occurredAt;
            }
    ...
    {
        $operation = ProductionOrderOperation::query()->findOrFail($operationId);
        if (in_array($operation->status, ['PLANNED', 'CANCELLED'], true)) {
            throw new DomainException('Operation is not executable', 422);
        }
        $good = round((float) ($payload['quantity_good'] ?? 0), 6);
        $scrap = round((float) ($payload['quantity_scrapped'] ?? 0), 6);
        $rework = round((float) ($payload['quantity_rework'] ?? 0), 6);
        if ($good + $scrap + $rework <= 0) {
            throw new DomainException('At least one output quantity is required', 422);
        }
        $current = (float) $operation->quantity_processed;
        if ($current + $good + $scrap + $rework > (float) $operation->quantity_planned + 0.000001 && empty($payload['allow_excess'])) {
            throw new DomainException('Reported quantity exceeds planned quantity', 422);
        }
        ...
        if (! $hasLaterOperation) {
            $this->productionOrderService->registerFinishedOperationOutput($output, $userId);
        }
    ...
    private function validateTransition(ProductionOrderOperation $operation, string $eventType): void
    {
        ...
        if (! $valid) {
            throw new DomainException(sprintf('Invalid MES transition %s from status %s', $eventType, $operation->status), 422);
        }
        if (in_array($eventType, ['START', 'RESUME'], true) && (int) $operation->sequence > 1) {
            ...
            if ($pending) {
                throw new DomainException('Previous operation must be completed before starting this operation', 422);
            }
        }
    }
    ...
    {
        $events = ProductionOperationEvent::query()->where('production_order_operation_id', $operationId)->orderBy('occurred_at')->get();
        $productive = 0.0;
        $pause = 0.0;
        $activeSince = null;
        $pauseSince = null;
        foreach ($events as $event) {
            if ($event->event_type === 'START') {
                $activeSince = $event->occurred_at;
            } elseif ($event->event_type === 'PAUSE' && $activeSince) {
                $productive += $activeSince->diffInSeconds($event->occurred_at) / 60;
                $activeSince = null;
                $pauseSince = $event->occurred_at;
            } elseif ($event->event_type === 'RESUME') {
                if ($pauseSince) {
                    $pause += $pauseSince->diffInSeconds($event->occurred_at) / 60;
                } $pauseSince = null;
                $activeSince = $event->occurred_at;
            } elseif (in_array($event->event_type, ['STOP', 'COMPLETE', 'CANCEL'], true)) {
                if ($activeSince) {
                    $productive += $activeSince->diffInSeconds($event->occurred_at) / 60;
                } if ($pauseSince) {
                    $pause += $pauseSince->diffInSeconds($event->occurred_at) / 60;
        ...
        $last = $events->last();
        if ($activeSince && $last && in_array($last->event_type, ['STOP', 'COMPLETE', 'CANCEL'], true)) {
            $productive += $activeSince->diffInSeconds($last->occurred_at) / 60;
        }
