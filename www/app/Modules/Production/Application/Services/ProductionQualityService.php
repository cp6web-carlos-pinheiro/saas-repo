...
final class ProductionQualityService extends BaseService
{
    ...
    public function record(int $operationId, array $payload, ?int $userId = null): array
    {
        ...
        if ($quantity <= 0) {
            throw new DomainException('Quality quantity must be positive', 422);
        }
        if ($quantity > (float) $operation->quantity_processed - (float) $operation->quantity_scrapped - (float) $operation->quantity_rework + 0.000001 && empty($payload['allow_excess'])) {
            throw new DomainException('Quality quantity exceeds available processed quantity', 422);
        }
    ...
    public function createRework(int $operationId, array $payload, ?int $userId = null): array
    {
        $operation = ProductionOrderOperation::query()->findOrFail($operationId);
        $quantity = round((float) $payload['quantity'], 6);
        if ($quantity <= 0 || $quantity > (float) $operation->quantity_rework + 0.000001 && empty($payload['allow_excess'])) {
            throw new DomainException('Rework quantity is invalid for the operation', 422);
        }
