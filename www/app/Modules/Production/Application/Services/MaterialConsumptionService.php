...
final class MaterialConsumptionService extends BaseService
{
    ...
    public function record(int $productionOrderId, array $payload, ?int $operatorId = null): array
    {
        ...
        if (! $bomItem && isset($payload['reference_bom_component_id'])) {
            throw new DomainException('The selected BOM component does not belong to the consumed product', 422);
        }
        if (! $bomItem && empty($payload['allow_unplanned'])) {
            throw new DomainException('Consumed product is not present in the frozen BOM snapshot; set allow_unplanned to register additional consumption', 422);
        }
        ...
        if ($idempotencyKey) {
            ...
            if ($previous) {
                return $previous->refresh()->load(['product', 'warehouse'])->toArray();
            }
        }
        ...
        if ($alreadyConsumed + $quantityConsumed > (float) $plannedBomQuantity + 0.000001 && empty($payload['allow_excess'])) {
            throw new DomainException('Consumption exceeds the frozen BOM requirement', 422);
        }
    ...
    {
        if (trim($reason) === '') {
            throw new DomainException('A reason is required to reverse material consumption', 422);
        }
        ...
        if ($existing) {
            return $existing->load('consumption')->toArray();
        }
        if ($consumption->reversed_by_movement_id) {
            throw new DomainException('Consumption has already been reversed', 422);
        }
    ...
    public function aggregateByProduct(int $productionOrderId): array
    {
        ...
        foreach ($consumptions as $item) {
            ...
            if (! $item->reversed_by_movement_id) {
