...
final class MrpSuggestionWorkflowService extends BaseService
{
    ...
    public function persist(array $payload, array $result, ?int $userId = null): array
    {
        ...
        if ($existing) return ['plan_run' => $existing->toArray(), 'suggestions' => $existing->suggestions->toArray(), 'result' => $result];
    ...
    public function runs(int $perPage = 15): LengthAwarePaginator { return MrpPlanRun::query()->withCount('suggestions')->orderByDesc('id')->paginate($perPage); }
    public function suggestions(int $perPage = 15, array $filters = []): LengthAwarePaginator { return MrpSuggestion::query()->with(['product', 'planRun'])->when($filters['status'] ?? null, fn ($q, $v) => $q->where('status', $v))->when($filters['suggestion_type'] ?? null, fn ($q, $v) => $q->where('suggestion_type', $v))->orderByDesc('id')->paginate($perPage); }
    public function show(int $id): array { return MrpSuggestion::query()->with(['product', 'planRun', 'events'])->findOrFail($id)->toArray(); }
    ...
    public function decide(int $id, string $status, array $payload, ?int $userId = null): array
    {
        ...
        if (in_array($suggestion->status, ['CONVERTED', 'CANCELLED'], true)) throw new DomainException('Suggestion is no longer actionable', 422);
    ...
    {
        $suggestion = MrpSuggestion::query()->findOrFail($id);
        if ($suggestion->production_order_id || $suggestion->purchase_requisition_id) return $suggestion->fresh()->toArray();
        if ($suggestion->status !== 'APPROVED') throw new DomainException('Only approved suggestions can be converted', 422);
