...
final class SupplierController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        abort_unless(in_array($sort, ['code', 'name', 'status', 'created_at'], true), 404);
        ...
        return view('client.suppliers.search', compact('suppliers', 'search', 'sort', 'direction', 'company'));
    }
    ...
    private function applySearchFilters(Builder $query, array $searchTerms): void
    {
        foreach ($searchTerms as $term) {
            $query->where(function (Builder $nested) use ($term): void {
                $nested->whereRaw('LOWER(code) like ?', ["%{$term}%"])
